...
class ParserTreasure(Parser):
    def parse(self):
        m = int(self.data[0])
        treasury = straw_hat2.StrawHatTreasury(m)
        for i in range(1, len(self.data)):
            ...
            if query_type == 'Add':
                try:
                    ...
                    print(OSError, f"Cannot add treasure {id} to treasury")
            elif query_type == 'Get':
                # try:
        ...
        for i in range(len(self.data)):
            ...
            if query[0] == 'Insert':
                try:
                    ...
                    print(OSError, f'Cannot insert {query[1]}')
            elif query[0] == 'Extract':
                try:
                    print(f'{h.extract()} extracted')
                    num -= 1
                except OSError:
                    print(OSError, 'Cannot extract')
            elif query[0] == 'Top':
                try:
                    print(f'Top: {h.top()}')
                except OSError :
                    print(OSError, 'Cannot get top')
            elif query[0] == 'Print':
                try:
                    for i in range(num):
                        print(h.extract(), end = ' ')
